...
export function DesktopReports() {
  ...
  return (
    <div className="min-h-screen animate-fade-in p-6 pb-20">
      <div className="mx-auto max-w-7xl space-y-8">
        {/* Header Section */}
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div className="text-left">
            <div className="mb-2 inline-flex items-center space-x-2 text-muted-foreground">
            ...
            <h1 className="bg-gradient-to-r from-primary via-cyan-600 to-blue-500 bg-clip-text text-4xl font-bold tracking-tight text-transparent">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {/* Period Selector */}
            <div className="flex space-x-1 overflow-x-auto rounded-xl border border-border-primary bg-background-elevated p-1">
              {periods.map((period) => (
                <button
                  key={period.id}
            ...
            {availableCurrencies.length > 2 && (
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="flex h-full min-w-[160px] items-center justify-between gap-2 rounded-xl border border-border-primary bg-background-elevated px-4 py-2.5 text-sm text-text-primary shadow-sm outline-none transition-colors hover:border-primary/50 focus:ring-2 focus:ring-primary/20"
                >
                  ...
                  <ArrowDownRight
                    className={`h-4 w-4 text-text-muted transition-transform duration-200 ${isCurrencyDropdownOpen ? 'rotate-180' : ''}`}
                  />
                </button>

                {isCurrencyDropdownOpen && (
                  <div className="absolute right-0 top-full z-50 mt-2 w-full min-w-[180px] origin-top-right animate-scale-in overflow-hidden rounded-xl border border-border-primary bg-background-elevated shadow-xl">
                    <div className="p-1">
        ...
        {selectedPeriod === 'custom' && (
          <div className="flex w-fit animate-fade-in items-center gap-4 rounded-xl border border-border-primary bg-background-elevated p-4">
            <div className="flex flex-col">
              <span className="mb-1 text-xs text-text-muted">Desde</span>
              <input
                type="date"
                ...
                className="rounded-lg border border-border-secondary bg-background-tertiary px-3 py-2 text-sm text-text-primary outline-none focus:border-primary"
              />
            </div>
            <div className="flex flex-col">
              <span className="mb-1 text-xs text-text-muted">Hasta</span>
              <input
                type="date"
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          <div className="group rounded-2xl border border-border-primary bg-background-elevated p-5 shadow-sm transition-colors hover:border-primary/30">
            <div className="mb-4 flex items-start justify-between">
              <div className="rounded-xl bg-green-500/10 p-2 text-green-500">
                <ArrowUpRight className="h-6 w-6" />
              </div>
              <span className="rounded-full bg-background-tertiary px-2 py-1 text-xs font-medium text-text-muted">
                Ingresos
              </span>
            </div>
          ...
          <div className="group rounded-2xl border border-border-primary bg-background-elevated p-5 shadow-sm transition-colors hover:border-primary/30">
            ...
            <div className="space-y-1">
              {totalsByCurrency.net.length > 0 ? (
                totalsByCurrency.net.map(({ currency, amount }) => (
                  <p
                    key={currency}
                    className={`text-2xl font-bold ${amount >= 0 ? 'text-text-primary' : 'text-red-500'}`}
                  >
                    {formatCurrency(amount, currency)}
                  </p>
                ))
            ...
            <p
              className={`mt-2 text-xs ${periodIncome - periodExpense >= 0 ? 'text-green-500' : 'text-red-500'}`}
            >
              {periodIncome - periodExpense >= 0 ? 'Superávit' : 'Déficit'}{' '}
              global
            </p>
          </div>
          ...
          <div className="group rounded-2xl border border-border-primary bg-background-elevated p-5 shadow-sm transition-colors hover:border-primary/30">
            ...
            <p
              className={`text-3xl font-bold ${savingsRate >= 20 ? 'text-green-500' : savingsRate > 0 ? 'text-yellow-500' : 'text-text-primary'}`}
            >
              {savingsRate.toFixed(1)}%
            </p>
            <div className="mt-3 h-1.5 w-full rounded-full bg-background-tertiary">
              <div
        ...
        {/* Main Chart Section */}
        <div className="rounded-3xl border border-border-primary bg-background-elevated p-6 shadow-sm">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-text-muted">
                Comportamiento {isLongPeriod ? 'mensual' : 'diario'} en el
                tiempo
              </p>
            </div>
                ...
                chartData.map((data, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className="relative w-full rounded-t-lg bg-blue-500/10"
                        style={{ height: `${Math.max(heightPercent, 2)}%` }}
                      >
                        <div className="group-hover:bg-primary-hover absolute bottom-0 h-full w-full rounded-t-lg bg-gradient-to-t from-primary/80 to-primary opacity-80 transition-all duration-500 group-hover:opacity-100"></div>
                      </div>
                      <span className="mt-2 block w-full truncate text-center text-[10px] text-text-muted">
                        {data.label}
                      </span>
                    </div>
                  );
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-1">
            ...
            <div className="rounded-3xl border border-border-primary bg-background-elevated p-6">
              ...
              <div className="flex h-32 items-end space-x-2">
                {weekDayChart.map((day, idx) => (
                  ...
                  >
                    <div className="pointer-events-none absolute bottom-full mb-1 rounded bg-background-tertiary px-2 py-1 text-[10px] opacity-0 shadow group-hover:opacity-100">
                      {formatCurrency(day.value, displayCurrency)}
                    </div>
                    <div
                      className="min-h-[4px] w-full rounded-t-sm bg-indigo-500/20"
                      style={{
                        height: `${maxWeekDay > 0 ? (day.value / maxWeekDay) * 100 : 0}%`,
                      }}
                    >
                      <div className="h-full w-full rounded-t-sm bg-indigo-500/80 transition-colors hover:bg-indigo-500"></div>
                    </div>
            ...
            <div className="rounded-3xl border border-border-primary bg-background-elevated p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between rounded-xl bg-background-tertiary/50 p-3">
                ...
                <div className="flex items-center justify-between rounded-xl bg-background-tertiary/50 p-3">
                  ...
                  <span className="font-semibold text-text-primary">
                    {periodIncome > 0
                      ? Math.round((periodExpense / periodIncome) * 100)
                      : 0}
                    %
                  </span>
                </div>
          ...
          <div className="h-fit rounded-3xl border border-border-primary bg-background-elevated p-6 lg:col-span-2">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-xl font-bold text-text-primary">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {categoryTotals.slice(0, 10).map((category, idx) => (
                <div
                  ...
                  className="rounded-2xl border border-transparent bg-background-tertiary/30 p-4 transition-all hover:border-border-secondary"
                >
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-background-primary">
                    <div
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {accountMetrics.length > 0 ? (
              accountMetrics.map((acc) => (
                <div
                  key={acc.id}
                  className="rounded-2xl border border-border-secondary bg-background-tertiary/30 p-5 transition-all hover:shadow-md"
                >
                  <div className="mb-3 flex items-start justify-between">
                    <h4
                      className="truncate pr-2 font-semibold text-text-primary"
                      title={acc.name}
                    >
                      {acc.name}
                    </h4>
                    <span className="rounded-md bg-background-primary px-2 py-1 font-mono text-xs text-text-muted">
                      {acc.currency}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="my-1 h-px w-full bg-border-secondary"></div>
                    <div className="flex justify-between text-sm">
                      <span className="font-medium text-text-primary">
                        Neto
                      </span>
                      <span
                        className={`font-bold ${acc.net >= 0 ? 'text-green-500' : 'text-red-500'}`}
                      >
