...
describe('AI Config', () => {
  ...
  describe('isQuotaExceededError', () => {
    ...
    it('should return true for AI SDK error messages', () => {
      ...
      expect(isQuotaExceededError({ message: 'Resource exhausted' })).toBe(
        true
      );
      expect(isQuotaExceededError({ message: 'User quota exceeded' })).toBe(
        true
      );
    });

    it('should return false for unrelated errors', () => {
      expect(isQuotaExceededError({ message: 'Internal Server Error' })).toBe(
        false
      );
