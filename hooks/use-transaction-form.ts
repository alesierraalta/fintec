...
export function useTransactionForm(): UseTransactionFormReturn {
  ...
  const [formData, setFormData] =
    useState<TransactionFormData>(INITIAL_FORM_DATA);
    ...
    try {
      ...
      if (formData.isRecurring) {
        try {
          ...
          alert(
            'Transacción creada pero hubo un error con la configuración recurrente.'
          );
        }
      }
      ...
      if (formData.isRecurring) {
        alert(
          `¡Transacción recurrente creada! Se repetirá cada ${
