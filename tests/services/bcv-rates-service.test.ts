...
describe('BCVRatesService', () => {
  ...
  describe('fetchRates', () => {
    it('should return fresh rates from API when successful', async () => {
      ...
      expect(bcvRatesService.getCachedRates()).toEqual(
    ...
    it('should use cached rates if available and API fails', async () => {
      ...
      expect(rates.fallbackReason).toBe('cache');
    });
  ...
  describe('getTrends', () => {
    it('should delegate to bcvHistoryService', async () => {
      ...
      (bcvHistoryService.calculateTrends as jest.Mock).mockResolvedValueOnce(
        mockTrends
      );
    ...
    it('should handle errors gracefully', async () => {
      (bcvHistoryService.calculateTrends as jest.Mock).mockRejectedValueOnce(
        new Error('DB Error')
      );
