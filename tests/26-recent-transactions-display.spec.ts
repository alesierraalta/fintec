...
test.describe('Recent Transactions Display @auth-required', () => {
  test('should display recent transactions on dashboard', async ({ page }) => {
    ...
    const recentTransactionsSection = page.locator(
    ...
    const hasNoTransactionsMessage = sectionContent.includes(
  ...
  test('should render RecentTransactions component with proper data binding', async ({
    page,
  }) => {
    // Navigate to dashboard
    await page.goto('/');
  ...
  test('should display transaction list or empty state correctly', async ({
    page,
  }) => {
    // Navigate to dashboard
    await page.goto('/');
    ...
    const transactionItems =
      recentTransactionsSection.locator('div[class*="flex"]');
  ...
  test('should verify RecentTransactions receives non-empty array when data exists', async ({
    page,
  }) => {
    ...
    const descriptionInput = page.locator(
      'input[name="description"], input[placeholder*="description" i]'
    );
    if (await descriptionInput.isVisible()) {
      ...
      const submitButton = page
        .locator(
          'button:has-text("Guardar"), button:has-text("Crear"), button:has-text("Save")'
        )
        .first();
