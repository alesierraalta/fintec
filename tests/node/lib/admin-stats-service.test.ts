...
let rejectedTable: string | null = null;
  ...
  users: [
    { id: 'u1', created_at: '2025-01-02T10:00:00.000Z', last_activity_at: '2025-01-31T12:00:00.000Z' },
    { id: 'u2', created_at: '2025-01-03T10:00:00.000Z', last_activity_at: '2025-01-31T12:00:00.000Z' },
    { id: 'u3', created_at: '2024-12-01T10:00:00.000Z', last_activity_at: null },
  ],
  accounts: [{ id: 'a1', user_id: 'u1' }, { id: 'a2', user_id: null }],
  transactions: [{ account_id: 'a1' }, { account_id: 'a2' }],
  budgets: [{ user_id: 'u1' }],
  goals: [{ user_id: 'u2' }],
  subscriptions: [{ user_id: 'u1' }],
  feedbacks: [{ user_id: 'u2' }],
  usage_tracking: [{ month_year: '2025-01', transaction_count: 2, backup_count: 3, api_calls: 4, export_count: 5, ai_requests: 6 }],
};
  ...
  return { from: jest.fn((table: string) => {
    const query: any = {
      ...
      then: (resolve: (value: unknown) => unknown) => Promise.resolve({
            ...
            error: rejectedTable === table ? { code: 'PGRST205', message: `Missing table ${table}` } : null,
          }).then(resolve),
    };
    return query;
  }) };
...
describe('admin stats service', () => {
  ...
  it('returns aggregate-only metrics with UTC activity and nullable transaction ownership excluded', async () => {
    ...
    expect(result.users.newByDay.find((bucket) => bucket.date === '2025-01-02')?.count).toBe(1);
    ...
    expect(result.resources.totals).toEqual({ accounts: 2, transactions: 1, budgets: 1, goals: 1, subscriptions: 1, feedbacks: 1 });
    expect(result.resources.perUserCounts).toEqual(expect.arrayContaining([
      expect.objectContaining({ userId: 'u1', accounts: 1, transactions: 1, budgets: 1 }),
      expect.objectContaining({ userId: 'u2', goals: 1, feedbacks: 1 }),
    ]));
    expect(result.usage.byMonth[0]).toEqual({ monthYear: '2025-01', transactionCount: 2, backupCount: 3, apiCalls: 4, exportCount: 5, aiRequests: 6 });
  ...
  it('degrades an unavailable feedbacks family without fabricating a zero', async () => {
        ...
        const result = await getAdminStats('30d');
        expect(result.resources.totals.feedbacks).toEqual({ status: 'unavailable', reason: 'query_failed' });
    ...
    expect(result.users).toMatchObject({ dau: 0, wau: 0, mau: 0, peakDailyActive: 0, peakDate: null, activityStatus: 'empty' });
  });
});
