...
export class FinanceDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      transactions:
        'id, type, accountId, categoryId, currencyCode, date, amountMinor, amountBaseMinor, transferId, createdAt, [accountId+date], [categoryId+date], [type+date], [date+type]',
      ...
      .upgrade(async (tx) => {
        await tx
          ...
          .modify((transaction: Transaction) => {
            if (
              transaction.isDebt === undefined ||
              transaction.isDebt === null
            ) {
              transaction.isDebt = false;
            }

            if (transaction.isDebt !== true) {
              transaction.debtDirection = undefined;
              transaction.debtStatus = undefined;
  ...
  async initialize(): Promise<void> {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (
          (isIndexError || isDatabaseClosedError) &&
  ...
  private async seedDefaultCategories(): Promise<void> {
    const defaultCategories: Omit<
      Category,
      'id' | 'createdAt' | 'updatedAt' | 'userId' | 'isDefault'
    >[] = [
      // Income categories
      {
        name: 'Salario',
    ...
    const categoriesToInsert: Category[] = defaultCategories.map(
      (cat, index) => ({
        ...cat,
        id: `cat_${index + 1}`,
        userId: null,
