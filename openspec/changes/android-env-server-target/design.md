...
to a new `android/app/src/debug/AndroidManifest.xml`, so debug builds keep plaintext
reachability for `http://10.0.2.2:3000`.

**Simply removing the attribute from the main manifest is not enough, and the first
attempt at this change was wrong on exactly that point.** The generated
`:capacitor-cordova-android-plugins` module declares
`android:usesCleartextTraffic="true"` in its own library manifest
(`android/capacitor-cordova-android-plugins/src/main/AndroidManifest.xml`). With no value
declared by the app, the library's value wins the merge and *every* variant — release
included — still permits plaintext HTTP to any host. Running the merger proved this: the
release manifest came out with `usesCleartextTraffic="true"` after the attribute had been
deleted from main.

The main manifest therefore pins the value explicitly:
...
`tools:replace` is required — without it the merger fails on the conflicting library
value rather than overriding it. The debug source set, which has higher priority than
main, overrides it back to `true` for local development.

That library manifest is regenerated by `cap sync`, so editing it directly is not an
option; the override has to live in the app.

**Verified by execution**, not by reasoning about merge semantics: the release blame
report records `ADDED from .../app/src/main/AndroidManifest.xml` and `REJECTED from
