...
The system SHOULD provide deterministic fixtures that combine debt and non-debt transactions across `INCOME` and `EXPENSE`, and tests SHOULD verify: directional debt totals, status-based inclusion/exclusion (`OPEN` vs `SETTLED`), non-regression parity for legacy transactions without debt metadata, and historical date rate accuracy in transaction details without silent fallbacks.
